...
{ 
    public class VirtualizingTreeViewItem : VirtualizingItemContainer
    {
        ...
        public TreeViewExpander Expander
        ...
        [SerializeField]
        private Text m_selectionTxt;

        [SerializeField]
        private Color m_m_selectionTxtColor = new Color32(0x0B, 0x7E, 0xF0, 0xFF);

        [SerializeField]
        private Color m_selectionNomalColor = new Color32(0x0B, 0x7E, 0xF0, 0xFF);
        ...
        {
            get { return m_isIsFocused; }
            set
            {
                if (m_isIsFocused != value)
                {
                    m_isIsFocused = value;
                    if (m_isIsFocused)
                    {
                        EyeButton.GetComponent<RectTransform>().anchoredPosition = new Vector2(350f, 0);
                        EyeButton.gameObject.SetActive(m_isIsFocused);
                    }
                    else
                    {
                        EyeButton.gameObject.SetActive(m_isIsFocused);
        ...
        private void UpdateGraphics()
        {
            if (m_selectionGraphics == null)
            {
                Debug.LogWarning("m_selectionGraphics is null.");
                return;
            }

            RectTransform rectTransform = m_selectionGraphics.GetComponent<RectTransform>();
            if (rectTransform != null)
            {
                Vector2 offsetMax = rectTransform.offsetMax;
                offsetMax.x = 250; // 设置右边距
                rectTransform.offsetMax = offsetMax;
            }

            if (EyeButton != null)
            {
                Image eyeButtonImage = EyeButton.GetComponent<Image>();
                if (eyeButtonImage != null)
                {
                    eyeButtonImage.color = IsSelected ? m_selectionFocusedColor : m_selectionNomalColor;
                }
            }
        }

        public void OnItemListAcitve(bool isActive) 
        {
            m_selectionTxt.color = isActive? Color.black : m_m_selectionTxtColor;
        }

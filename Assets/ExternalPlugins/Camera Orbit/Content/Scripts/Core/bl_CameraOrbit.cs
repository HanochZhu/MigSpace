...
public class bl_CameraOrbit : bl_CameraBase
{
    ...
    private float startRotateTime = 0;
    private Vector3 initialPosition, initialRotation;
    private IEnumerator currentTargetTransition;
    #endregion
    ...
    void SetUp()
    {
        ...
        // avoid the effect of 'teleport' in the first movement
        ...
        m_CurrentFOV = GetCamera.fieldOfView;
        m_DefaultFOV = m_CurrentFOV;
    ...
    {
        distance = DistanceClamp.x;
        TargetOffset = Vector3.zero;
    }

    public void OnMouseScroll()
    {
        if (Input.GetMouseButton(2)) // 检测鼠标中键是否被按下
        {
            float moveX = Input.GetAxis("Mouse X") * 0.4f; // 获取鼠标在X轴上的移动距离
            float moveY = Input.GetAxis("Mouse Y") * 0.4f; // 获取鼠标在Y轴上的移动距离

            // 将鼠标的移动转换成相机的上下左右位移
            Vector3 forward = Camera.main.transform.forward; // 相机的前方向
            Vector3 right = Camera.main.transform.right; // 相机的右方向

            forward.y = 0; // 忽略y轴，保持在水平面上
            right.y = 0;

            forward.Normalize(); // 归一化向量
            right.Normalize();

            Vector3 offset = new Vector3(-moveX, -moveY, 0); // 仅改变x和y值，z值保持为0
    ...
    void InputControl()
    {
        if (LockCursorOnRotate && !useKeys)
        {
            if (!isForMobile)
            {
                //did the user stop rotating the camera
                if (!isInputKeyRotate && LastHaveInput)
                {
                    OnStopRotating();
                    LastHaveInput = false;
                }
                //when the user start to rotate the camera
                if (isInputKeyRotate && !LastHaveInput)
                {
                    OnStartRotating();
                    LastHaveInput = true;
    ...
    void OnStopRotating()
    {
        ...
        float rotateTime = Time.time - startRotateTime;
        if (rotateTime < Time.deltaTime * 8) return;

        pointerDelta = Input.mousePosition - lastMousePosition;
        pointerDelta.x = pointerDelta.x / Screen.width;
        pointerDelta.y = pointerDelta.y / Screen.height;

        float userInterationSpeed = pointerDelta.magnitude / rotateTime;
        AutoRotSpeed = userInterationSpeed * OutInputSpeed;
        AutoRotSpeed = Mathf.Min(60, AutoRotSpeed);
    }
    ...
    void AutoRotation()
    {
        ...
        switch (AutoRotationType)
        {
            case CameraAutoRotationType.Dinamicaly:
                autoSpeed = (lastHorizontal > 0) ? ((defaultAutoSpeed + AutoRotSpeed) * Time.deltaTime) * 3 : -(((defaultAutoSpeed + AutoRotSpeed) * Time.deltaTime) * 3);
                break;
            case CameraAutoRotationType.Left:
                autoSpeed = ((defaultAutoSpeed + AutoRotSpeed) * Time.deltaTime) * 3;
                break;
        ...
        horizontal += autoSpeed;
        if (AutoRotSpeed > 0)
        {
            AutoRotSpeed -= Time.deltaTime * 12;
            AutoRotSpeed = Mathf.Max(0, AutoRotSpeed);
        }
    }
    ...
    void FOVControl()
    {
        ...
        m_CurrentFOV = Mathf.SmoothStep(m_CurrentFOV, m_DefaultFOV, Time.deltaTime * FovLerp);
        //smooth transition with lerp
        GetCamera.fieldOfView = Mathf.Lerp(GetCamera.fieldOfView, m_CurrentFOV, Time.deltaTime * FovLerp);
    }
    ...
    void ZoomControll(bool autoApply)
    {
        ...
        distance = Mathf.Clamp(distance - (MouseScrollWheel * ScrollSensitivity),0,999);
        ...
        if (DetectCollision)
        {
            ...
            Ray = new Ray(TargetPosition, forward);
            RaycastHit hit;
    ...
    {
        StopCoroutine(nameof(TranslateTarget));
        StartCoroutine(nameof(TranslateTarget), newTarget);
    }

    /// <summary>
    /// Change the camera orbit target with a smooth transition
    /// </summary>
    /// <param name="target"></param>
    public void SetTarget(bl_OrbitTargetPlaceholder target)
    {
        if (Target == target.Target) return;

        if(currentTargetTransition != null) StopCoroutine(currentTargetTransition);
        currentTargetTransition = TransitionToHolder();
        StartCoroutine(currentTargetTransition);

        /// <summary>
        /// 
        /// </summary>
        IEnumerator TransitionToHolder()
        {
             ...
             if(target.GetCameraLocation(out var position, out var rotation))
             {
                 SetCameraToLocation(position, rotation);
             }
    ...
    /// <summary>
    /// Change the orbit camera position and rotation to the given position or rotation
    /// This method will automatically adjust the position and rotation to the nearest orbit trajectory
    /// </summary>

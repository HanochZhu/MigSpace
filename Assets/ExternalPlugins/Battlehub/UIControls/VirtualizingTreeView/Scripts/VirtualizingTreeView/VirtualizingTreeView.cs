...
namespace Battlehub.UIControls
{
    ...
    public class VirtualizingTreeView : VirtualizingItemsControl<VirtualizingTreeViewItemDataBindingArgs>
    {
        ...
        {
            base.Start();
            EventManager.StartListening(MigEventCommon.OnClickModel, OnInputHitModelChangeList);
        }
        ...
        {
            base.OnDisable();
            EventManager.StopListening(MigEventCommon.OnClickModel, OnInputHitModelChangeList);
        }
        public void OnTreeListItemActive(object arg0)
        {
            GameObject obj = (GameObject)arg0;

            VirtualizingTreeViewItem childContainer = (VirtualizingTreeViewItem)GetItemContainer(arg0);
            childContainer.isActive = !childContainer.isActive;
            childContainer.OnItemListAcitve(childContainer.isActive);
            obj.SetActive(childContainer.isActive);
        }
        private void OnInputHitModelChangeList(object arg0, object arg1)
        {
            if (arg0 == null)
            {
                return;
            ...
            {
                VirtualizingTreeViewItem childContainer = (VirtualizingTreeViewItem)GetItemContainer(parentObjects[i]);                  
                if (childContainer == null)
                    continue;
                else
                {
                    childContainer.Expander.IsOn = true;
                }
            }
            VirtualizingTreeViewItem childContainer1 = (VirtualizingTreeViewItem)GetItemContainer(arg0);
            if (childContainer1 != null)
                childContainer1.IsSelected = true;

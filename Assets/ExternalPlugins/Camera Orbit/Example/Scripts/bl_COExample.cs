...
public class bl_COExample : MonoBehaviour {
    ...
    void Start()
    {
        CurrenTragetText.text = placeholdersTargets[0].Target.name;
    }
    ...
    public void ChangeTarget(bool b)
    {
        if (b)
        {
            CurrentTarget = (CurrentTarget + 1) % placeholdersTargets.Length;
        }
        else
        {
            if (CurrentTarget > 0)
            ...
            {
                CurrentTarget = placeholdersTargets.Length - 1;
            }
        }
        Orbit.SetTarget(placeholdersTargets[CurrentTarget]);
        CurrenTragetText.text = placeholdersTargets[CurrentTarget].Target.name;
    }
}
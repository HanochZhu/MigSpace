...
namespace RTG
{
    ...
    public class ObjectTransformGizmo : GizmoBehaviour
    {
        ...
        private bool _scaleConstraintEnabled = false;
        private Vector3 _minPositiveScale = Vector3Ex.FromValue(1e-5f);
        ...
        public bool ScaleConstraintEnabled { get { return _scaleConstraintEnabled; } set { _scaleConstraintEnabled = value; } }
        ...
        {
            EventManager.TriggerEvent(MigEventCommon.OnGizmosDragBegin);
        ...
        {
            EventManager.TriggerEvent(MigEventCommon.OnGizmosDragUpdate);
        ...
        {
            EventManager.TriggerEvent(MigEventCommon.OnGizmosDragEnd); 
        ...
        private void ScaleObjects()
        {
            ...
            if (TransformPivot == GizmoObjectTransformPivot.CustomObjectLocalPivot)
            {
                foreach (var trParent in _transformableParents)
                {
                    Transform objectTransform = trParent.transform;
                    Vector3 worldPivot = objectTransform.TransformPoint(GetObjectCustomLocalPivot(trParent));
                    ScaleObject(trParent, worldPivot);
                }
            }
        }
        ...
        private void ScaleObject(GameObject gameObject, Vector3 scalePivot)
        {
            ...
            if (ScaleConstraintEnabled)
            {
                Vector3 totalDragScale = Gizmo.TotalDragScale;
                if (totalDragScale[0] < 0.0f) scaleFactor[0] = 1.0f;
                if (totalDragScale[1] < 0.0f) scaleFactor[1] = 1.0f;
                if (totalDragScale[2] < 0.0f) scaleFactor[2] = 1.0f;

                objectTransform.ScaleFromPivot(scaleFactor, scalePivot);
                objectTransform.localScale = Vector3.Max(objectTransform.localScale, _minPositiveScale);
            }
